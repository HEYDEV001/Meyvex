...
export const Component = ({
  respondWhileTyping = false,
}: {
  /**
   * By default the keyboard ignores keystrokes typed into inputs/textareas,
   * so it doesn't visually react while a user fills out an unrelated form
   * elsewhere on the page. Set true to have it respond to typing anywhere,
   * including inside form fields.
   */
  respondWhileTyping?: boolean;
} = {}) => {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (!respondWhileTyping && isTypingTarget(event.target)) return;
  ...
  }, [activateKey, deactivateKey, respondWhileTyping]);
